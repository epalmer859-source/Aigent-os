...
function _dbRowToQueueRow(row: DbQueueRow): QueueRow {
  return {
    ...
    scheduledSendAt: row.scheduled_send_at ?? new Date(),
...
async function _processQueueFromDb(batchSize: number): Promise<QueueWorkerResult> {
  ...
  const claimedRows = await db.$transaction(async (tx) => {
    const rows = await tx.$queryRaw<DbQueueRow[]>`
      ...
      WHERE (
        (q.status = 'pending' AND (q.scheduled_send_at IS NULL OR q.scheduled_send_at <= NOW())
      ...
      ORDER BY q.scheduled_send_at ASC NULLS FIRST
      LIMIT ${batchSize}
...
async function _processDeferredMessagesFromDb(): Promise<QueueWorkerResult> {
  ...
  const claimedRows = await db.$transaction(async (tx) => {
    const rows = await tx.$queryRaw<DbQueueRow[]>`
      ...
      FROM outbound_queue q
      LEFT JOIN conversations c ON c.id = q.conversation_id
      WHERE q.status = 'pending' AND (q.scheduled_send_at IS NULL OR q.scheduled_send_at <= NOW())
        AND (q.claim_token IS NULL OR q.claim_expires_at < NOW())
      ORDER BY q.scheduled_send_at ASC NULLS FIRST
      LIMIT 100
      FOR UPDATE OF q SKIP LOCKED
...
async function _processRowFromDb(row: QueueRow, db: any): Promise<keyof QueueWorkerResult | null> {
  // F8: Send-time verification for scheduling messages
  if (row.schedulingJobId && row.messagePurpose.startsWith("scheduling_")) {
    const { verifySendTimeState } = await import("../scheduling/send-time-verify");
    const { createSendTimeVerifyDb } = await import("../scheduling/prisma-scheduling-adapter");
    const verifyDb = createSendTimeVerifyDb(db);
    const verification = await verifySendTimeState(
      row.schedulingJobId,
      row.messagePurpose as any,
      verifyDb,
    );
    if (!verification.shouldSend) {
      await db.outbound_queue.update({
        where: { id: row.id },
        data: { status: "canceled", terminal_failure_reason: `send_time_verify: ${verification.reason} (current: ${verification.currentStatus ?? "unknown"})` },
      });
      return "suppressed";
    }
  }
