...
import { z } from "zod";
import {
  PLACE_APPOINTMENT_VALID_STATES,
  NO_SHOW_VALID_STATES,
  CLOSEOUT_BLOCKING_TAGS,
  URGENT_PURPOSES_EXEMPT_FROM_PAUSE,
  type ActorContext,
  type AdminActionResult,
  type PlaceAppointmentParams,
  type RescheduleAppointmentParams,
  type CancelAppointmentParams,
  type AssignTechnicianParams,
  type MarkEnRouteParams,
  type MarkDelayedParams,
  type MarkJobInProgressParams,
  type MarkJobPausedParams,
  type MarkJobCompleteParams,
...
const _appointments = new Map<string, AppointmentRecord>();
...
const _noShowValidSet = new Set<string>(NO_SHOW_VALID_STATES);
const _placeApptValidSet = new Set<string>(PLACE_APPOINTMENT_VALID_STATES);
...
function _cancelRowsByAppointmentId(appointmentId: string): number {
  let count = 0;
  for (const row of _queueRows.values()) {
    if (
...
function _getApptForAction(
  appointmentId: string,
): AppointmentRecord | null {
  return _appointments.get(appointmentId) ?? null;
}

function _getConvByAppt(
  appt: AppointmentRecord,
): ConversationRecord | null {
  return _conversations.get(appt.conversationId) ?? null;
}
...
export async function placeAppointment(
  actor: ActorContext,
  params: PlaceAppointmentParams,
): Promise<AdminActionResult> {
  z.object({ conversationId: z.string().min(1), appointmentDate: z.string().min(1), appointmentTime: z.string().min(1), serviceType: z.string().min(1) }).parse(params);
  if (process.env.NODE_ENV !== "test") return _placeAppointmentFromDb(actor, params);

  const conv = _getConvForAction(params.conversationId);
  if (!conv) return _failResult("place_appointment", "Conversation not found", params.conversationId);

  if (!_placeApptValidSet.has(conv.primaryState)) {
    return _failResult("place_appointment", `Invalid state: ${conv.primaryState}`, params.conversationId);
  }

  // Production: db.$transaction([...])
  const apptId = _genId("appt");
  const address = params.address ?? conv.collectedServiceAddress ?? null;
  ...
  const canceled = _cancelRowsByConversationAndPurposes(params.conversationId, [
    "stale_waiting_internal_ping",
    "routine_followup_1",
    "routine_followup_final",
  ]);

  conv.primaryState = "booked";
  _queueMessage(params.conversationId, "booking_confirmation", actor.businessId);
  _logEvent(params.conversationId, "appointment_marked_booked");

  return _okResult("place_appointment", {
    conversationId: params.conversationId,
    stateChanged: true,
    newState: "booked",
    notificationsQueued: ["booking_confirmation"],
    queueRowsCanceled: canceled,
    eventLogged: "appointment_marked_booked",
  });
}
...
export async function rescheduleAppointment(
  ...
  z.object({ appointmentId: z.string().min(1), newDate: z.string().min(1), newTime: z.string().min(1) }).parse(params);
  if (process.env.NODE_ENV !== "test") return _rescheduleAppointmentFromDb(actor, params);

  const appt = _getApptForAction(params.appointmentId);
  if (!appt) return _failResult("reschedule_appointment", "Appointment not found");

  const conv = _getConvByAppt(appt);
  if (!conv) return _failResult("reschedule_appointment", "Conversation not found");

  // Production: db.$transaction([...])
  appt.appointmentDate = params.newDate;
  appt.appointmentTime = params.newTime;

  const canceled = _cancelRowsByAppointmentId(params.appointmentId);
  conv.primaryState = "booked";

  _queueMessage(appt.conversationId, "reschedule_confirmation", actor.businessId, params.appointmentId);
  _logEvent(appt.conversationId, "appointment_marked_rescheduled");

  return _okResult("reschedule_appointment", {
    conversationId: appt.conversationId,
    stateChanged: true,
    newState: "booked",
    notificationsQueued: ["reschedule_confirmation"],
    queueRowsCanceled: canceled,
    eventLogged: "appointment_marked_rescheduled",
  });
}
...
export async function cancelAppointment(
  ...
  z.object({ appointmentId: z.string().min(1) }).parse(params);
  if (process.env.NODE_ENV !== "test") return _cancelAppointmentFromDb(actor, params);

  const appt = _getApptForAction(params.appointmentId);
  if (!appt) return _failResult("cancel_appointment", "Appointment not found");

  // Production: db.$transaction([...])
  appt.status = "canceled";
  const canceled = _cancelRowsByAppointmentId(params.appointmentId);

  _queueMessage(appt.conversationId, "cancellation_confirmation", actor.businessId);
  _logEvent(appt.conversationId, "appointment_marked_canceled");

  return _okResult("cancel_appointment", {
    conversationId: appt.conversationId,
    notificationsQueued: ["cancellation_confirmation"],
    queueRowsCanceled: canceled,
    eventLogged: "appointment_marked_canceled",
  });
}
...
export async function assignTechnician(
  ...
  z.object({ appointmentId: z.string().min(1), technicianName: z.string().min(1) }).parse(params);
  if (process.env.NODE_ENV !== "test") return _assignTechnicianFromDb(actor, params);

  const appt = _getApptForAction(params.appointmentId);
  if (!appt) return _failResult("assign_technician", "Appointment not found");

  const conv = _getConvByAppt(appt);
  if (!conv) return _failResult("assign_technician", "Conversation not found");

  // Production: db.$transaction([...])
  appt.technicianName = params.technicianName;

  const wasBooked = conv.primaryState === "booked";
  if (wasBooked) conv.primaryState = "tech_assigned";

  _logEvent(appt.conversationId, "technician_assigned");

  return _okResult("assign_technician", {
    conversationId: appt.conversationId,
    stateChanged: wasBooked,
    newState: wasBooked ? "tech_assigned" : undefined,
    eventLogged: "technician_assigned",
  });
}
...
export async function markEnRoute(
  ...
  z.object({ appointmentId: z.string().min(1) }).parse(params);
  if (process.env.NODE_ENV !== "test") return _markEnRouteFromDb(actor, params);

  const appt = _getApptForAction(params.appointmentId);
  if (!appt) return _failResult("mark_en_route", "Appointment not found");

  const conv = _getConvByAppt(appt);
  if (!conv) return _failResult("mark_en_route", "Conversation not found");

  // Production: db.$transaction([...])
  conv.primaryState = "en_route";
  _queueMessage(appt.conversationId, "dispatch_notice", actor.businessId);
  _logEvent(appt.conversationId, "dispatch_marked_en_route");

  return _okResult("mark_en_route", {
    conversationId: appt.conversationId,
    stateChanged: true,
    newState: "en_route",
    notificationsQueued: ["dispatch_notice"],
    eventLogged: "dispatch_marked_en_route",
  });
}
...
export async function markDelayed(
  ...
  z.object({ appointmentId: z.string().min(1) }).parse(params);
  if (process.env.NODE_ENV !== "test") return _markDelayedFromDb(actor, params);

  const appt = _getApptForAction(params.appointmentId);
  if (!appt) return _failResult("mark_delayed", "Appointment not found");

  // Production: db.$transaction([...])
  _queueMessage(appt.conversationId, "delay_notice", actor.businessId);
  _logEvent(appt.conversationId, "dispatch_marked_delayed");

  return _okResult("mark_delayed", {
    conversationId: appt.conversationId,
    notificationsQueued: ["delay_notice"],
    eventLogged: "dispatch_marked_delayed",
  });
}
...
export async function markJobInProgress(
  ...
  z.object({ appointmentId: z.string().min(1) }).parse(params);
  if (process.env.NODE_ENV !== "test") return _markJobInProgressFromDb(actor, params);

  const appt = _getApptForAction(params.appointmentId);
  if (!appt) return _failResult("mark_job_in_progress", "Appointment not found");

  const conv = _getConvByAppt(appt);
  if (!conv) return _failResult("mark_job_in_progress", "Conversation not found");

  conv.primaryState = "job_in_progress";
  _logEvent(appt.conversationId, "job_marked_in_progress");

  return _okResult("mark_job_in_progress", {
    conversationId: appt.conversationId,
    stateChanged: true,
    newState: "job_in_progress",
    eventLogged: "job_marked_in_progress",
  });
}
...
export async function markJobPaused(
  ...
  z.object({ appointmentId: z.string().min(1), reason: z.string().min(1) }).parse(params);
  if (process.env.NODE_ENV !== "test") return _markJobPausedFromDb(actor, params);

  const appt = _getApptForAction(params.appointmentId);
  if (!appt) return _failResult("mark_job_paused", "Appointment not found");

  const conv = _getConvByAppt(appt);
  if (!conv) return _failResult("mark_job_paused", "Conversation not found");

  conv.primaryState = "job_paused";
  _logEvent(appt.conversationId, "job_marked_paused");

  return _okResult("mark_job_paused", {
    conversationId: appt.conversationId,
    stateChanged: true,
    newState: "job_paused",
    eventLogged: "job_marked_paused",
  });
}
...
export async function markJobComplete(
  ...
  z.object({ appointmentId: z.string().min(1) }).parse(params);
  if (process.env.NODE_ENV !== "test") return _markJobCompleteFromDb(actor, params);

  const appt = _getApptForAction(params.appointmentId);
  if (!appt) return _failResult("mark_job_complete", "Appointment not found");

  const conv = _getConvByAppt(appt);
  if (!conv) return _failResult("mark_job_complete", "Conversation not found");

  conv.primaryState = "job_completed";

  const queued: string[] = [];
  const hasBlockingTag = conv.tags.some((t) => _closeoutBlockingSet.has(t));
  if (!hasBlockingTag) {
    _queueMessage(appt.conversationId, "closeout", actor.businessId);
    queued.push("closeout");
  }
  _queueMessage(appt.conversationId, "payment_management_ready", actor.businessId);
  queued.push("payment_management_ready");

  _logEvent(appt.conversationId, "job_marked_complete");

  return _okResult("mark_job_complete", {
    conversationId: appt.conversationId,
    stateChanged: true,
    newState: "job_completed",
    notificationsQueued: queued,
    eventLogged: "job_marked_complete",
  });
}
...
export async function markNoShow(
  ...
  z.object({ appointmentId: z.string().min(1) }).parse(params);
  if (process.env.NODE_ENV !== "test") return _markNoShowFromDb(actor, params);

  const appt = _getApptForAction(params.appointmentId);
  if (!appt) return _failResult("mark_no_show", "Appointment not found");

  const conv = _getConvByAppt(appt);
  if (!conv) return _failResult("mark_no_show", "Conversation not found");

  if (!_noShowValidSet.has(conv.primaryState)) {
    return _failResult("mark_no_show", `Invalid state: ${conv.primaryState}`, conv.id);
  }
  ...
  const canceled = _cancelAllPendingForConversation(conv.id);

  _logEvent(conv.id, "appointment_marked_no_show");

  return _okResult("mark_no_show", {
    conversationId: conv.id,
    stateChanged: true,
    newState: "resolved",
    queueRowsCanceled: canceled,
    eventLogged: "appointment_marked_no_show",
  });
}
...
async function _placeAppointmentFromDb(
  actor: ActorContext,
  params: PlaceAppointmentParams,
): Promise<AdminActionResult> {
  ...
  const conv = await db.conversations.findUnique({
    ...
    select: { id: true, customer_id: true, primary_state: true, collected_service_address: true, channel: true },
  });
  if (!conv) return _failResult("place_appointment", "Conversation not found", params.conversationId);
  if (!_placeApptValidSet.has(conv.primary_state as string)) {
    return _failResult("place_appointment", `Invalid state: ${conv.primary_state}`, params.conversationId);
  }
  const conversationId = params.conversationId;
  const address = params.address ?? conv.collected_service_address ?? null;
  ...
  const { canceledCount } = await db.$transaction(async (tx) => {
    await tx.appointments.create({
      data: {
        ...
        appointment_date: new Date(params.appointmentDate + "T00:00:00.000Z"),
        appointment_time: new Date(`1970-01-01T${params.appointmentTime}Z`),
    ...
    const { count } = await tx.outbound_queue.updateMany({
      where: {
        conversation_id: conversationId,
        message_purpose: { in: ["stale_waiting_internal_ping", "routine_followup_1", "routine_followup_final"] },
    ...
    await tx.outbound_queue.create({
      data: {
        ...
        dedupe_key: `${conversationId}_booking_confirmation_${Date.now()}`,
    ...
    await tx.event_log.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        event_code: "appointment_marked_booked",
...
async function _rescheduleAppointmentFromDb(
  actor: ActorContext,
  params: RescheduleAppointmentParams,
): Promise<AdminActionResult> {
  const { db } = await import("~/server/db");
  ...
  if (!appt?.conversation_id) return _failResult("reschedule_appointment", "Appointment not found");
  ...
  const { canceledCount } = await db.$transaction(async (tx) => {
    await tx.appointments.update({
      where: { id: params.appointmentId },
      data: {
        appointment_date: new Date(params.newDate + "T00:00:00.000Z"),
        appointment_time: new Date(`1970-01-01T${params.newTime}Z`),
    ...
    const { count } = await tx.outbound_queue.updateMany({
      where: {
        conversation_id: conversationId,
        message_purpose: { in: ["booking_confirmation", "reschedule_confirmation"] },
        status: { in: ["pending", "deferred"] },
      },
      data: { status: "canceled" as any },
    });
    ...
    await tx.outbound_queue.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        message_purpose: "reschedule_confirmation",
        audience_type: "customer" as any,
        channel: conv.channel as any,
        dedupe_key: `${conversationId}_reschedule_confirmation_${Date.now()}`,
        scheduled_send_at: new Date(),
      },
    });
    await tx.event_log.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        event_code: "appointment_marked_rescheduled",
        event_family: "admin_action" as any,
        source_actor: src as any,
      },
    });
...
async function _cancelAppointmentFromDb(
  actor: ActorContext,
  params: CancelAppointmentParams,
): Promise<AdminActionResult> {
  const { db } = await import("~/server/db");
  ...
  if (!appt?.conversation_id) return _failResult("cancel_appointment", "Appointment not found");
  const conversationId = appt.conversation_id;
  const conv = await db.conversations.findUnique({
    where: { id: conversationId },
    select: { channel: true },
  });
  if (!conv) return _failResult("cancel_appointment", "Conversation not found");
  const src = actor.role === "owner" ? "owner" : "admin_team";
  const { canceledCount } = await db.$transaction(async (tx) => {
    await tx.appointments.update({
      where: { id: params.appointmentId },
      data: { status: "canceled" as any, canceled_at: new Date() },
    });
    const { count } = await tx.outbound_queue.updateMany({
      where: {
        conversation_id: conversationId,
        message_purpose: { in: ["booking_confirmation", "reschedule_confirmation", "dispatch_notice"] },
        status: { in: ["pending", "deferred"] },
      },
      data: { status: "canceled" as any },
    });
    await tx.outbound_queue.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        message_purpose: "cancellation_confirmation",
        audience_type: "customer" as any,
        channel: conv.channel as any,
        dedupe_key: `${conversationId}_cancellation_confirmation_${Date.now()}`,
        scheduled_send_at: new Date(),
      },
    });
    await tx.event_log.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        event_code: "appointment_marked_canceled",
        event_family: "admin_action" as any,
        source_actor: src as any,
      },
    });
...
async function _assignTechnicianFromDb(
  actor: ActorContext,
  params: AssignTechnicianParams,
): Promise<AdminActionResult> {
  const { db } = await import("~/server/db");
  ...
  if (!appt?.conversation_id) return _failResult("assign_technician", "Appointment not found");
  const conversationId = appt.conversation_id;
  const conv = await db.conversations.findUnique({
    where: { id: conversationId },
  ...
  if (!conv) return _failResult("assign_technician", "Conversation not found");
  const wasBooked = (conv.primary_state as string) === "booked";
  const src = actor.role === "owner" ? "owner" : "admin_team";
  await db.$transaction(async (tx) => {
    await tx.appointments.update({
      where: { id: params.appointmentId },
      data: { technician_name: params.technicianName },
    });
    if (wasBooked) {
      await tx.conversations.update({
        where: { id: conversationId },
        data: { primary_state: "tech_assigned" as any },
      });
    }
    await tx.event_log.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        event_code: "technician_assigned",
        event_family: "admin_action" as any,
        source_actor: src as any,
      },
    });
...
async function _markEnRouteFromDb(
  actor: ActorContext,
  params: MarkEnRouteParams,
): Promise<AdminActionResult> {
  const { db } = await import("~/server/db");
  ...
  if (!appt?.conversation_id) return _failResult("mark_en_route", "Appointment not found");
  const conversationId = appt.conversation_id;
  const conv = await db.conversations.findUnique({
    where: { id: conversationId },
    select: { channel: true },
  ...
  await db.$transaction(async (tx) => {
    await tx.conversations.update({
      where: { id: conversationId },
      data: { primary_state: "en_route" as any },
    });
    await tx.outbound_queue.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        message_purpose: "dispatch_notice",
        audience_type: "customer" as any,
        channel: conv.channel as any,
        dedupe_key: `${conversationId}_dispatch_notice_${Date.now()}`,
        scheduled_send_at: new Date(),
      },
    });
    await tx.event_log.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        event_code: "dispatch_marked_en_route",
        event_family: "admin_action" as any,
        source_actor: src as any,
      },
    });
...
async function _markDelayedFromDb(
  actor: ActorContext,
  params: MarkDelayedParams,
): Promise<AdminActionResult> {
  const { db } = await import("~/server/db");
  ...
  if (!appt?.conversation_id) return _failResult("mark_delayed", "Appointment not found");
  const conversationId = appt.conversation_id;
  const conv = await db.conversations.findUnique({
    where: { id: conversationId },
    select: { channel: true },
  });
  if (!conv) return _failResult("mark_delayed", "Conversation not found");
  const src = actor.role === "owner" ? "owner" : "admin_team";
  await db.$transaction(async (tx) => {
    await tx.outbound_queue.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        message_purpose: "delay_notice",
        audience_type: "customer" as any,
        channel: conv.channel as any,
        dedupe_key: `${conversationId}_delay_notice_${Date.now()}`,
        scheduled_send_at: new Date(),
      },
    });
    await tx.event_log.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        event_code: "dispatch_marked_delayed",
        event_family: "admin_action" as any,
        source_actor: src as any,
      },
    });
...
async function _markJobInProgressFromDb(
  actor: ActorContext,
  params: MarkJobInProgressParams,
): Promise<AdminActionResult> {
  const { db } = await import("~/server/db");
  ...
  if (!appt?.conversation_id) return _failResult("mark_job_in_progress", "Appointment not found");
  const conversationId = appt.conversation_id;
  const src = actor.role === "owner" ? "owner" : "admin_team";
  await db.$transaction(async (tx) => {
    await tx.conversations.update({
      where: { id: conversationId },
      data: { primary_state: "job_in_progress" as any },
    });
    await tx.event_log.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        event_code: "job_marked_in_progress",
        event_family: "admin_action" as any,
        source_actor: src as any,
      },
    });
...
async function _markJobPausedFromDb(
  actor: ActorContext,
  params: MarkJobPausedParams,
): Promise<AdminActionResult> {
  const { db } = await import("~/server/db");
  ...
  if (!appt?.conversation_id) return _failResult("mark_job_paused", "Appointment not found");
  const conversationId = appt.conversation_id;
  const src = actor.role === "owner" ? "owner" : "admin_team";
  await db.$transaction(async (tx) => {
    await tx.conversations.update({
      where: { id: conversationId },
      data: { primary_state: "job_paused" as any },
    });
    await tx.event_log.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        event_code: "job_marked_paused",
        event_family: "admin_action" as any,
        source_actor: src as any,
      },
    });
...
async function _markJobCompleteFromDb(
  actor: ActorContext,
  params: MarkJobCompleteParams,
): Promise<AdminActionResult> {
  const { db } = await import("~/server/db");
  ...
  if (!appt?.conversation_id) return _failResult("mark_job_complete", "Appointment not found");
  const conversationId = appt.conversation_id;
  const conv = await db.conversations.findUnique({
    where: { id: conversationId },
    select: { channel: true },
  ...
  if (!conv) return _failResult("mark_job_complete", "Conversation not found");
  const blockingTag = await db.conversation_tags.findFirst({
    where: { conversation_id: conversationId, tag_code: { in: Array.from(_closeoutBlockingSet) }, is_active: true },
  });
  const hasBlockingTag = !!blockingTag;
  const src = actor.role === "owner" ? "owner" : "admin_team";
  const queued: string[] = [];
  await db.$transaction(async (tx) => {
    await tx.appointments.update({
      where: { id: params.appointmentId },
      data: { status: "completed" as any, completed_at: new Date() },
    });
    await tx.conversations.update({
      where: { id: conversationId },
      data: { primary_state: "job_completed" as any },
    });
    if (!hasBlockingTag) {
      await tx.outbound_queue.create({
        data: {
          business_id: actor.businessId,
          conversation_id: conversationId,
          message_purpose: "closeout",
          audience_type: "customer" as any,
          channel: conv.channel as any,
          dedupe_key: `${conversationId}_closeout_${Date.now()}`,
          scheduled_send_at: new Date(),
        },
      });
      queued.push("closeout");
    ...
    await tx.outbound_queue.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        message_purpose: "payment_management_ready",
        audience_type: "internal" as any,
        channel: conv.channel as any,
        dedupe_key: `${conversationId}_payment_management_ready_${Date.now()}`,
        scheduled_send_at: new Date(),
      },
    });
    queued.push("payment_management_ready");
    await tx.event_log.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        event_code: "job_marked_complete",
        event_family: "admin_action" as any,
        source_actor: src as any,
      },
    });
...
async function _markNoShowFromDb(
  actor: ActorContext,
  params: MarkNoShowParams,
): Promise<AdminActionResult> {
  const { db } = await import("~/server/db");
  ...
  if (!appt?.conversation_id) return _failResult("mark_no_show", "Appointment not found");
  const conversationId = appt.conversation_id;
  const conv = await db.conversations.findUnique({
    where: { id: conversationId },
    select: { id: true, primary_state: true },
  ...
  if (!conv) return _failResult("mark_no_show", "Conversation not found");
  if (!_noShowValidSet.has(conv.primary_state as string)) {
    return _failResult("mark_no_show", `Invalid state: ${conv.primary_state}`, conv.id);
  }
  const src = actor.role === "owner" ? "owner" : "admin_team";
  const { canceledCount } = await db.$transaction(async (tx) => {
    await tx.appointments.update({
    ...
    await tx.conversations.update({
      where: { id: conversationId },
      data: { primary_state: "resolved" as any, is_no_show: true, closed_at: new Date() },
    });
    const { count } = await tx.outbound_queue.updateMany({
    ...
    await tx.event_log.create({
      data: {
        business_id: actor.businessId,
        conversation_id: conversationId,
        event_code: "appointment_marked_no_show",
        event_family: "admin_action" as any,
        source_actor: src as any,
      },
    });
...
export function _seedAppointmentForTest(data: Record<string, unknown>): void {
  ...
  _appointments.set(data["id"] as string, data as unknown as AppointmentRecord);
}
...
export function _getAppointmentForTest(id: string): AppointmentRecord | undefined {
  return _appointments.get(id);
}

export function _getAppointmentByConversationForTest(
  conversationId: string,
