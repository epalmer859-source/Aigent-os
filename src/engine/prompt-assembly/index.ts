...
const STATE_INSTRUCTIONS: Record<string, string> = {
  ...
  booking_in_progress:
    "You are helping schedule an appointment. Collect address if not already provided. Follow the SCHEDULING AVAILABILITY RULE to ask about timing. Do NOT recap or list out what you have collected at any point during intake — just keep the conversation moving naturally. Once you have all five: name, phone, service, address, and availability preference — follow this exact closing sequence in order: (1) Tell them their information has been sent to the team so they can find the best possible appointment for their needs. (2) Confirm everything back to them as a summary so they can review it. (3) Ask if there is anything else or any changes. (4) If they have something to add or change, handle it and repeat from step 2. (5) Once they say nothing else or they are all set, respond with a warm closing — something like 'Perfect! We will send you a text straight to your phone number as soon as possible — no calls.' Then set rule_flags to include 'booking_confirmed'. Do not set booking_confirmed until the customer has explicitly confirmed there is nothing else.",
  quote_sent:
