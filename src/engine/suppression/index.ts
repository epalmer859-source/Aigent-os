...
interface AppointmentChangeRequestRecord {
  id: string;
  conversationId: string;
  businessId: string;
...
const _appointmentChangeRequests = new Map<string, AppointmentChangeRequestRecord>();
...
function _checkPerPurpose(
  ...
  if (p in ALLOWED_STATES_BY_PURPOSE) {
    ...
    // Appointment reminders: block on accepted change request.
    // Production: SELECT id FROM appointment_change_requests
    //   WHERE conversation_id = ? AND request_status = 'accepted_from_customer'
    //   LIMIT 1
    if (p === "appointment_reminder_24h" || p === "appointment_reminder_3h") {
      for (const req of _appointmentChangeRequests.values()) {
        if (req.conversationId === convId && req.requestStatus === "accepted_from_customer") {
          return { decision: "suppress", reason: "appointment_change_requested" };
        }
      }
    }
...
export function _resetSuppressionStoreForTest(): void {
  ...
  _appointmentChangeRequests.clear();
...
async function _shouldSendMessageFromDb(context: MessageContext): Promise<SuppressionResult> {
  ...
  if (!isInternal && convRow) {
    ...
    if (p in ALLOWED_STATES_BY_PURPOSE) {
      ...
      // Appointment reminder: check for accepted change request
      if (p === "appointment_reminder_24h" || p === "appointment_reminder_3h") {
        const acr = await db.appointment_change_requests.findFirst({
          where: { conversation_id: context.conversationId, request_status: "accepted_from_customer" },
          select: { id: true },
        });
        if (acr) return { decision: "suppress", reason: "appointment_change_requested" };
      }
    ...
    const _humanTakeoverBlocked = new Set(["missed_call_fallback","booking_confirmation","reschedule_confirmation","cancellation_confirmation","dispatch_notice","delay_notice","schedule_change_notice","quote_delivery","admin_response_relay","handoff_response"]);
