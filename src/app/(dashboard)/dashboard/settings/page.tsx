...
const TABS: { value: Tab; label: string }[] = [
  ...
  { value: "quotes", label: "Quotes & Auto-Close" },
...
function parseLanguages(raw: string | null | undefined): Record<string, LanguageEntry> {
  ...
  try {
    ...
    return { ...DEFAULT_LANGUAGES, ...parsed, english: { ...DEFAULT_LANGUAGES.english, ...(parsed.english ?? { enabled: true, speaker: "" }), enabled: true } };
  } catch {
    // Legacy plain text (e.g. "English, Spanish") — return defaults
        ...
        return (
          <div key={lang.key} className="space-y-1.5">
            <label className="flex items-center gap-2 text-sm text-gray-700">
              ...
              {lang.locked && <span className="text-xs text-gray-400">(always enabled)</span>}
            </label>
            {entry.enabled && (
              <input
                ...
                value={entry.speaker}
                onChange={(e) =>
                  onChange({ ...value, [lang.key]: { ...entry, speaker: e.target.value } })
                }
      ...
      {DAYS.map((day) => {
        ...
        return (
          <div key={day} className="flex items-center gap-3">
            <span className="w-24 shrink-0 text-xs font-medium capitalize text-gray-700">
              {day}
            </span>
            <input
              type="checkbox"
              checked={!h.closed}
              onChange={(e) =>
                onChange({ ...value, [day]: { ...h, closed: !e.target.checked } })
              }
            ...
            {!h.closed && (
              <>
                <input
                  ...
                  value={h.open}
                  onChange={(e) =>
                    onChange({ ...value, [day]: { ...h, open: e.target.value } })
                  }
                ...
                <input
                  ...
                  value={h.close}
                  onChange={(e) =>
                    onChange({ ...value, [day]: { ...h, close: e.target.value } })
                  }
                  className="rounded border border-gray-200 px-2 py-1 text-xs"
                />
              </>
    ...
    <div className="space-y-2">
      {value.map((svc, i) => (
        <div key={i} className="flex items-center gap-2">
          <span className="flex-1 rounded-lg border border-gray-100 bg-gray-50 px-3 py-1.5 text-sm text-gray-900">
            {svc.name}
          </span>
          <button
            onClick={() => remove(i)}
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (cfg) {
      ...
      try {
        setServiceAreaRaw(JSON.stringify(cfg.service_area_list ?? [], null, 2));
      } catch { /* empty */ }
    ...
    <div className="flex flex-col gap-4">
        ...
        <Section title="Business Info">
          <fieldset disabled={!isOwner} className="m-0 min-w-0 border-0 p-0">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2 space-y-2">
                <label className="flex items-center gap-2.5 text-xs font-medium text-gray-700">
                  <input
                    type="checkbox"
          ...
          {isOwner && (
            <div className="mt-4 flex items-center gap-3">
              ...
              />
              {updateBiz.isError && (
                <p className="text-xs text-red-500">{updateBiz.error.message}</p>
              )}
            </div>
          )}
        </Section>
      ...
      {tab === "ai" && (
        <Section title="AI Behavior">
          ...
          <div className="mt-4">
            <p className="mb-2 text-xs font-medium text-gray-700">Supported Languages</p>
          ...
          {isOwner && (
            <div className="mt-4 flex items-center gap-3">
              ...
              />
              {updateAi.isError && (
                <p className="text-xs text-red-500">{updateAi.error.message}</p>
              )}
            </div>
          )}
        </Section>
        ...
        <Section title="Policies & Instructions">
          ...
          {isOwner && (
            <div className="mt-4 flex items-center gap-3">
              ...
              />
              {updatePol.isError && (
                <p className="text-xs text-red-500">{updatePol.error.message}</p>
              )}
            </div>
          )}
        </Section>
          ...
          {isOwner && (
            <div className="mt-4 flex items-center gap-3">
              <SaveBtn
                onClick={() =>
                  updateQuotes.mutate({
                    quote_expiry_days: quoteExpiry,
                    auto_close_days: autoClose,
                  })
                }
              ...
              {updateQuotes.isError && (
                <p className="text-xs text-red-500">{updateQuotes.error.message}</p>
              )}
            </div>
          )}
        </Section>
      ...
      {tab === "team" && (
        <div className="space-y-4">
          <Section title="Team Members">
            {!team || team.length === 0 ? (
              ...
              <div className="space-y-2">
                {team.map((user) => (
                  <div
                    key={user.id}
                    className="flex items-center justify-between gap-2 rounded-lg border border-gray-100 bg-gray-50 px-4 py-3"
                  >
                    <div>
            ...
            {changeRole.isError && (
              <p className="mt-2 text-xs text-red-500">{changeRole.error.message}</p>
            )}
            {removeUser.isError && (
              <p className="mt-2 text-xs text-red-500">{removeUser.error.message}</p>
            )}
          </Section>
          <Section title="Invite Admins">
            <p className="text-sm text-gray-600">
              Share your join code:{" "}
              <span className="font-mono font-bold text-gray-900">{data?.join_code}</span>
            </p>
            <button
        ...
        <Section title="Danger Zone">
          {isOwner ? (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                <SaveBtn
                  onClick={() =>
                    pauseMutation.mutate({
                      isPaused,
                      pauseMessage: pauseMsg || undefined,
                    })
                  }
                ...
                {pauseMutation.isError && (
                  <p className="text-xs text-red-500">{pauseMutation.error.message}</p>
                )}
              </div>
            </div>
          ) : (
            <p className="text-sm text-gray-500">
              Business status:{" "}
              <span className={isPaused ? "font-medium text-red-600" : "font-medium text-green-600"}>
                {isPaused ? "Paused" : "Active"}
              </span>
              {isPaused && pauseMsg && (
                <span className="ml-1 text-gray-400">— {pauseMsg}</span>
              )}
            </p>
          )}
        </Section>
